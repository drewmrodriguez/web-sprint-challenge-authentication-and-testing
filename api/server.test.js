...
describe('server.js', () => {
  test('[1] responds with the correct message on valid credentials', async () => {
    const res = await request(server).post('/api/auth/login').send({ username: 'drew', password: 'rodriguez' })
    expect(res.body.message).toMatch(/welcome drew/i);
  }, 500);

  test('[2] responds with a token with correct { username,  exp, iat }', async () => {
    let res = await request(server).post('/api/auth/login').send({ username: 'drew', password: 'rodriguez' });
...
describe('[GET] /api/users', () => {
  ...
  test('[3] requests with an invalid token are bounced with proper status and message', async () => {
    const res = await request(server).get('/api/jokes').set('Authorization', 'luis');
